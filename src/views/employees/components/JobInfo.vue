      ...
      <div class="block">
        ...
        <el-form-item label="HRBP">
          <el-select v-model="formData.hrbp" filterable placeholder="请选择" class="inputW">
            <el-option v-for="item in employees" :key="item.id" :label="item.username" :value="item.id" class="inputW" />
          </el-select>
        </el-form-item>
...
import { employeeJobInfo, updateJob, SimpleEmployees } from '@/api/employees.js'
...
export default {
  ...
  created() {
    this.getJobInfo()
    this.SimpleEmployees()
  },
  methods: {
    async getJobInfo() {
      this.formData = await employeeJobInfo(this.$route.params.id)
    },
    async saveJob() {
      await updateJob(this.formData).then(() => {
        this.$message.success('更新成功')
      })
    },
    async SimpleEmployees() {
      this.employees = await SimpleEmployees()
    }
  }
}
</script>
