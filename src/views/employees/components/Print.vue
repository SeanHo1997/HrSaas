      ...
      <el-card>
        <el-breadcrumb separator="/" class="titInfo ">
          ...
          <el-row type="flex" justify="end">
            <el-button v-print="printObj" type="primary" size="mini">打印</el-button>
          </el-row>
          <!-- /打印按钮 -->
        </el-breadcrumb>
        <div v-if="type === 'personal'" id="printObj">
...
<script>
import { employeePersonalInfo, employeeJobInfo } from '@/api/employees'
...
export default {
  ...
  // 创建完毕状态
  created() {
    this.type === 'personal' ? this.getPersonalDetail() : this.employeeJobInfo()
  },
  // 组件更新
  methods: {
    async getPersonalDetail() {
      this.formData = await employeePersonalInfo(this.userId) // 获取个人基本信息
    },
    async employeeJobInfo() {
      const userInfo = await userBasicInfo(this.userId)
      const jobInfo = await employeeJobInfo(this.userId) // 获取个人基本信息
