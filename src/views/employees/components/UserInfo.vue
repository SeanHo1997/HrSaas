    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="生日">
          <el-input v-model="formData.birthday" placeholder="示例 0323" class="input" />
        </el-form-item>
...
import { employeePersonalInfo, savePersonalInfo } from '@/api/employees.js'
...
export default {
  ...
  methods: {
    async getBasiclInfo() {
      this.userInfo = await userBasicInfo(this.$route.params.id)
      if (this.userInfo.staffPhoto) {
        this.$refs.imgupload1.fileList = [{ url: this.userInfo.staffPhoto, uploaded: true }]
      }
    },
    async getPersonalInfo() {
      this.formData = await employeePersonalInfo(this.$route.params.id)
      if (this.formData.staffPhoto) {
        this.$refs.imgupload2.fileList = [{ url: this.formData.staffPhoto, uploaded: true }]
      }
    },
    async saveUser() {
      const fileList = this.$refs.imgupload1.fileList
      await saveBasicInfo({ ...this.userInfo, staffPhoto: fileList.length ? fileList[0].url : '' }).then(() => {
        this.$message.success('更新成功')
      })
    },
    async savePersonal() {
      const fileList = this.$refs.imgupload2.fileList
      await savePersonalInfo({ ...this.formData, staffPhoto: fileList.length ? fileList[0].url : '' }).then(() => {
        this.$message.success('更新成功')
      })
    }
  }
